...
function renderRelated(item){const related=all.filter(x=>x.id!==item.id&&x.status!=='tam-an').slice(0,3);$('#relatedGrid').innerHTML=related.map(x=>{const slug=x.slug||`${slugify(x.title)}-${x.id}`,img=normalizeImageUrl(x.image)||'../assets/images/video-poster.jpg';return `<a class="related-card" href="/p/?id=${encodeURIComponent(x.id)}&slug=${encodeURIComponent(slug)}"><img src="${esc(img)}" alt="${esc(x.title)}"><div><h3>${esc(x.title)}</h3><strong>${esc(x.price||'Liên hệ')}</strong></div></a>`}).join('')||'<p>Chưa có sản phẩm liên quan.</p>'}
