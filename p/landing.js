...
function embedMap(url=''){if(!url)return ''; if(url.includes('output=embed'))return url; const q=encodeURIComponent(url); return `https://www.google.com/maps?q=${q}&output=embed`}
...
if(item.map){$('#mapLink').href=item.map;$('#mapWrap').innerHTML=`<iframe class="map-frame" src="${esc(embedMap(item.map))}" loading="lazy"></iframe>`}else{$('#mapSection').hidden=true}
